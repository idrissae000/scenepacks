...
interface Theme {
  ...
  font: string; gradient: string; heroOverlay: string; label: string; tagline: string
}
        ...
        <div className="relative z-10 mx-auto max-w-5xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            >
              ...
              <div className="relative aspect-[3/4] rounded-sm overflow-hidden flex items-center justify-center"
                style={{ background: t.surface, border: `1px solid ${t.border}` }}>
                {/* Top/bottom letterbox bars */}
                <div className="absolute top-0 left-0 right-0 h-8" style={{ background: t.bg, zIndex: 2 }} />
                <div className="absolute bottom-0 left-0 right-0 h-8" style={{ background: t.bg, zIndex: 2 }} />

                <span className="text-[8rem] font-black select-none"
                  style={{
                ...
                {['top-0 left-0', 'top-0 right-0', 'bottom-0 left-0', 'bottom-0 right-0'].map((pos, idx) => (
                  <div key={idx} className={`absolute ${pos} w-8 h-8 z-10`} style={{
                    borderTop: idx < 2 ? `2px solid ${t.gold}60` : 'none',
                    borderBottom: idx >= 2 ? `2px solid ${t.gold}60` : 'none',
                    borderLeft: idx % 2 === 0 ? `2px solid ${t.gold}60` : 'none',
                    borderRight: idx % 2 === 1 ? `2px solid ${t.gold}60` : 'none',
                  }} />
                ))}

                <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                  <div className="text-xs tracking-widest uppercase" style={{ color: t.gold, opacity: 0.6 }}>
                    {t.label}
                  </div>
                </div>
              </div>
            ...
            >
              ...
              {/* Pack details */}
              <div className="rounded-sm p-4" style={{ background: t.surface, border: `1px solid ${t.border}` }}>
                <div className="mob-label mb-3" style={{ color: t.gold }}>Pack Includes</div>
                <ul className="space-y-1.5 text-xs" style={{ color: t.muted }}>
                  {['High-quality scene clips', 'Cinematic color grading', 'Multiple aspect ratios', 'Free to use for edits'].map((item) => (
                    <li key={item} className="flex items-center gap-2">
                      <span style={{ color: t.gold }}>—</span> {item}
                    </li>
                  ))}
                </ul>
                <p className="text-xs mt-3 italic" style={{ color: t.muted, opacity: 0.6 }}>
                  * Pack details placeholder — update with real content
                </p>
              </div>
              ...
              <a
                ...
                className="flex items-center justify-center gap-3 py-4 rounded-sm font-bold text-sm tracking-widest uppercase transition-all duration-200"
                style={{
              ...
              <Link
                ...
                className="text-center text-xs tracking-widest uppercase transition-opacity hover:opacity-80"
                style={{ color: t.muted }}
              >
                ← Back to {parent.name}
              </Link>
