...
export default function Navbar() {
  ...
  useEffect(() => { setDropdownOpen(false); setUserMenuOpen(false) }, [pathname])
          ...
          <div className="px-4 py-4 space-y-1">
            ...
            {!session ? (
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-sm px-5 py-3 w-full font-bold"
                style={{ background: '#5865F2', color: '#ffffff', border: 'none', cursor: 'pointer', fontFamily: 'Inter, system-ui, sans-serif', fontSize: '1rem', fontWeight: 700 }}
              >
                <DiscordIcon /> Log In with Discord
              </button>
            ) : (
              <div style={{ borderTop: '1px solid #2a1410', paddingTop: '0.5rem', marginTop: '0.25rem' }}>
                <p className="px-4 py-2 text-sm" style={{ color: '#d4c5a9', fontFamily: 'Inter, system-ui, sans-serif' }}>
                  {session.user?.name}
                </p>
                <button
                  onClick={() => { signOut(); setMobileOpen(false) }}
                  className="block w-full text-left px-4 py-3"
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#847464', fontFamily: 'Inter, system-ui, sans-serif', fontSize: '1rem' }}
                >
                  Sign Out
                </button>
              </div>
