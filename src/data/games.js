const DISCORD = 'https://discord.com/invite/98C5YUeEz7'
const IDRISS = '349045631656001537'

export const games = [
  {
    id: 1,
    name: 'Red Dead Redemption 2',
    slug: 'rdr2',
    blurb: "An epic tale of life in America's unforgiving heartland. The end of the outlaw era.",
    theme: {
      bg: '#0f0905', surface: '#1a0f06',
      accent: '#8b3a0f', accentLight: '#b04f18', highlight: '#c9a84c',
      text: '#d4c5a9', muted: '#8a7560', border: '#3d1f0a',
      cardText: '#d4c5a9', cardMuted: '#9a8570',
      headingFont: '"Playfair Display", Georgia, serif',
      bodyFont: '"IM Fell English", Georgia, serif',
      texClass: '', cardClass: 'card-rdr2',
      bgImage: '/backgrounds/rdr2bg.jpg',
      logo: '/logos/rdr2logo.jpg',
      atmosphere: [], loadFx: null, titleFx: null,
      label: 'American Frontier, 1899', tagline: "We are thieves in a world that don't want us no more.",
      cursor: 'default',
    },
    characters: [
      {
        id: 'arthur-morgan',
        name: 'Arthur Morgan',
        slug: 'arthur-morgan',
        image: '/characters/arthur.png',
        description: "The soul of the Van der Linde gang. Honor, loyalty, and a world that keeps asking too much.",
        packLink: DISCORD,
        dateAdded: '2026-06-06',
        driveLink: 'https://drive.google.com/drive/folders/1WhcQNcassX5iuMB5bIlX21xGJXWkhOYy',
        packs: [
          { label: 'Aesthetic Pack', image: '/character-pages/arthuraesthetic.png', packLink: 'https://drive.google.com/drive/folders/1WhcQNcassX5iuMB5bIlX21xGJXWkhOYy', creatorId: IDRISS },
          { label: 'Sad Pack',       image: '/character-pages/arthursad.png',       packLink: 'https://drive.google.com/drive/folders/1dS8D5ESp_3xSDCquFPdVfyc41yaAOnVY', creatorId: IDRISS },
        ],
      },
      {
        id: 'micah-bell',
        name: 'Micah Bell',
        slug: 'micah-bell',
        image: '/characters/micahh.png',
        pageImage: '/character-pages/micahscp.png',
        description: "The rot inside the gang. Every scene he's in makes your skin crawl.",
        packLink: DISCORD,
        dateAdded: '2026-06-05',
        driveLink: 'https://drive.google.com/drive/folders/1hOvCpvhORtWTk_hGKg48tV37kgUOIaLY',
        creatorId: IDRISS,
      },
      {
        id: 'dutch-van-der-linde',
        name: 'Dutch van der Linde',
        slug: 'dutch-van-der-linde',
        image: '/characters/dutchpic.png',
        pageImage: '/character-pages/dutchscp.png',
        description: "The dreamer who led them all. A man with a plan, until the plan fell apart.",
        packLink: DISCORD,
        dateAdded: '2026-06-12',
        driveLink: 'https://drive.google.com/drive/folders/1_nOfVIc8cOjAt5GBKVxRqbcxVlt89RLG',
        creatorId: IDRISS,
      },
    ],
  },
  {
    id: 2,
    name: 'Batman: Arkham Series',
    slug: 'batman-arkham',
    blurb: "The Dark Knight faces Gotham's most dangerous villains across four groundbreaking games.",
    theme: {
      bg: '#080a0f', surface: '#0f1a2a',
      accent: '#1a3a5c', accentLight: '#2a5a8c', highlight: '#c9a84c',
      text: '#d4c5a9', muted: '#7a8a9a', border: '#1a2a3a',
      cardText: '#d4c5a9', cardMuted: '#8a9aaa',
      headingFont: '"Bebas Neue", system-ui, sans-serif',
      bodyFont: '"Bebas Neue", system-ui, sans-serif',
      texClass: 'tex-gotham', cardClass: 'card-arkham',
      bgImage: '/backgrounds/arkhambg.jpeg',
      logo: '/logos/arkhamlogo.jpg',
      atmosphere: [], loadFx: null, titleFx: null,
      label: 'Gotham City', tagline: 'I am the night.',
      cursor: 'default',
    },
    characters: [
      {
        id: 'batman-arkham',
        name: 'Batman — Arkham Series',
        slug: 'batman-arkham',
        image: '/characters/arkhambatman.png',
        pageImage: '/character-pages/arkhambatmanscp.png',
        description: "The Dark Knight at his most brutal. Gotham's protector across Asylum, City, Origins and Knight.",
        packLink: DISCORD,
        dateAdded: '2026-06-09',
        driveLink: 'https://drive.google.com/drive/folders/1oj0SbDNZACvE1MU2ee0sgIx1cXRh4m0b',
        creatorId: IDRISS,
      },
      {
        id: 'joker-arkham',
        name: 'The Joker',
        slug: 'the-joker',
        image: '/characters/arkhamjokerpic.png',
        pageImage: '/character-pages/arkhamjokerscp.png',
        description: "Chaos incarnate. Mark Hamill's Joker in the Arkham series remains the most terrifying version of the character ever put to a screen.",
        packLink: 'https://discord.com/invite/98C5YUeEz7',
        dateAdded: '2026-06-10',
        driveLink: 'https://drive.google.com/drive/folders/1xvZyTSfZeHlOvbGBt1SIYjR7vXbdMajM',
        creatorId: IDRISS,
      },
      {
        id: 'arkham-knight',
        name: 'Arkham Knight',
        slug: 'arkham-knight',
        image: '/characters/arkhamknightpic.png',
        pageImage: '/character-pages/arkhamknightscp.png',
        description: "Gotham's most personal enemy. A war machine built from Batman's greatest failure.",
        packLink: DISCORD,
        dateAdded: '2026-06-13',
        driveLink: 'https://drive.google.com/drive/folders/1LojkkVXqoY8vRerAu4JlBg1wvCf1NyMC',
        creatorId: IDRISS,
      },
    ],
  },
  {
    id: 3,
    name: 'God of War',
    slug: 'god-of-war',
    blurb: "The Ghost of Sparta carries the weight of his past through worlds of myth, blood and fatherhood.",
    theme: {
      bg: '#0a0505', surface: '#110808',
      accent: '#8b0000', accentLight: '#cc2200', highlight: '#c0c0c0',
      text: '#d4c5a9', muted: '#7a6a5a', border: '#3a1010',
      cardText: '#d4c5a9', cardMuted: '#9a8a7a',
      headingFont: '"Cinzel", Georgia, serif',
      bodyFont: '"Cinzel", Georgia, serif',
      texClass: '', cardClass: 'card-gow',
      bgImage: '/backgrounds/godofwarbg.jpg',
      logo: '/logos/godofwarlogo.jpg',
      atmosphere: [], loadFx: null, titleFx: null,
      label: 'The Nine Realms', tagline: 'Bear witness.',
      cursor: 'default',
    },
    characters: [
      {
        id: 'kratos',
        name: 'Kratos',
        slug: 'kratos',
        image: '/characters/kratospic.png',
        pageImage: '/character-pages/kratosscp.png',
        description: "The Ghost of Sparta. Forged in war, cursed by the gods, defined by what he protects.",
        packLink: DISCORD,
        dateAdded: '2026-06-13',
        driveLink: 'https://drive.google.com/drive/folders/1mdeU1r8mNyWbSGwcfnMob71f2xFPGPwf',
        creatorId: IDRISS,
      },
    ],
  },
  {
    id: 5,
    name: 'Grand Theft Auto V',
    slug: 'gta-v',
    blurb: "Three criminals risk everything in a series of heists across Los Santos. Chaos, money, and survival.",
    theme: {
      bg: '#080a08', surface: '#0f110f',
      accent: '#1a7a1a', accentLight: '#229922', highlight: '#c9a84c',
      text: '#ffffff', muted: '#8a9a8a', border: '#1a3a1a',
      cardText: '#ffffff', cardMuted: '#8a9a8a',
      headingFont: '"Bebas Neue", system-ui, sans-serif',
      bodyFont: '"Bebas Neue", system-ui, sans-serif',
      texClass: '', cardClass: 'card-clean',
      bgImage: '/backgrounds/GTAVbg.jpg',
      logo: '/logos/GTAVlogo.jpg',
      atmosphere: [], loadFx: null, titleFx: null,
      label: 'Los Santos, San Andreas', tagline: 'The pursuit of the almighty dollar.',
      cursor: 'default',
    },
    characters: [
      {
        id: 'michael-de-santa',
        name: 'Michael De Santa',
        slug: 'michael-de-santa',
        image: '/characters/michaelpic.png',
        pageImage: '/character-pages/michaelscp.png',
        description: "A retired criminal pulled back into the life he tried to leave. Brilliant, volatile, and deeply unhappy with everything he has.",
        packLink: 'https://discord.gg/MVA5ySY2',
        dateAdded: '2026-07-13',
        driveLink: 'https://drive.google.com/drive/folders/1D3TUhFW13YCdrSlVdnNbcPJr0h4SCMcE?usp=sharing',
        creatorId: IDRISS,
      },
      {
        name: 'Trevor Philips',
        slug: 'trevor-philips',
        image: '/characters/trevor_pic.png',
        pageImage: '/character-pages/trevor_scp.png',
        description: "Unhinged, unpredictable and somehow the most honest character in the game. Trevor does what everyone else only thinks about.",
        packLink: 'https://discord.com/invite/98C5YUeEz7',
        dateAdded: '2026-07-20',
        driveLink: 'https://drive.google.com/drive/folders/1nqCgl0EfDKTc6fMjrD-vvvMJb1rulxG-?usp=sharing',
        creatorId: IDRISS,
      },
      {
        name: 'Franklin Clinton',
        slug: 'franklin-clinton',
        image: '/characters/franklin_pic.png',
        pageImage: '/character-pages/franklin_scp.png',
        description: "The hungry one. A young man from the streets trying to leave it all behind — until he can't.",
        packLink: 'https://discord.com/invite/98C5YUeEz7',
        dateAdded: '2026-07-20',
        driveLink: 'https://drive.google.com/drive/folders/1mEH8qe5mlcT3bD8JwppPAtmsMbjyTsQG?usp=sharing',
        creatorId: IDRISS,
      },
    ],
  },
  {
    id: 4,
    name: '007 First Light',
    slug: '007-first-light',
    blurb: "A sleek espionage thriller following a young James Bond as he earns his license to kill.",
    theme: {
      bg: '#0a0a0f', surface: '#1a2a3a',
      accent: '#1a2a3a', accentLight: '#2a3a5a', highlight: '#c9a84c',
      text: '#d4c5a9', muted: '#7a8a9a', border: '#1a2a3a',
      cardText: '#d4c5a9', cardMuted: '#8a9aaa',
      headingFont: '"Bebas Neue", system-ui, sans-serif',
      bodyFont: '"Bebas Neue", system-ui, sans-serif',
      texClass: '', cardClass: 'card-007',
      bgImage: '/backgrounds/007firstlightbg.jpg',
      logo: '/logos/007firstlightlogo.png',
      atmosphere: [], loadFx: null, titleFx: null,
      label: 'MI6 — License to Kill', tagline: 'Bond. James Bond.',
      cursor: 'default',
    },
    characters: [
      {
        id: 'james-bond',
        name: 'James Bond',
        slug: 'james-bond',
        image: '/characters/jamesbondpic.png',
        pageImage: '/character-pages/jamesbondscp.png',
        description: "Agent 007. Cool under pressure, lethal when it counts, and always one step ahead.",
        packLink: 'https://discord.gg/MVA5ySY2',
        dateAdded: '2026-06-24',
        driveLink: 'https://drive.google.com/drive/folders/1et-6ivtS79PFlQw-79F_oBQtSMBocAuR?usp=sharing',
        creatorId: IDRISS,
      },
    ],
  },
]
